...
data$group <- factor(data$group, labels = c("MDD", "HC"))
...
## switch dysfunctional CERQ items into negative scores

```{r}

data <- data %>%
  mutate(CERQ_self_blame_neg = if_else(CERQ_self_blame > 0, CERQ_self_blame * -1, CERQ_self_blame),
         CERQ_rumination_neg = if_else(CERQ_rumination > 0, CERQ_rumination * -1, CERQ_rumination),
         CERQ_catastrophizing_neg = if_else(CERQ_catastrophizing > 0, CERQ_catastrophizing * -1, CERQ_catastrophizing),
         CERQ_blaming_others_neg = if_else(CERQ_blaming_others > 0, CERQ_blaming_others * -1, CERQ_blaming_others))
...
```{r}
scores_scaled <- scale(data[, c("CERQ_self_blame_neg", "CERQ_rumination_neg", "CERQ_catastrophizing_neg", "CERQ_blaming_others_neg", "CERQ_putting_into_perspective", "CERQ_positive_reinforcement", "CERQ_positive_reappraisal", "CERQ_refocus_on_planning", "CERQ_acceptance")])
...
```{r}
cerq_scores_by_cluster <- data %>%
  group_by(cluster) %>%
  summarise(across(ends_with("score"),mean,na.rm = TRUE))

```

```{r}
print(cerq_scores_by_cluster)
```

```{r}
library(ggplot2)

cerq_scores_by_cluster_long <- cerq_scores_by_cluster %>%
  pivot_longer(-cluster, names_to = "CERQ_Score", values_to = "Value")

ggplot(cerq_scores_by_cluster_long, aes(x = cluster, y = Value, fill = CERQ_Score)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  theme_minimal() +
  labs(title = "Mean CERQ Scores by Cluster",
       x = "Cluster",
       y = "Mean Score") +
