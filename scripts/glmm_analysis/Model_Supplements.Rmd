...
## load dataset
```{r}
data <- read_excel("/Users/julia/Desktop/Git/EEGfMRIStudy/data/BackwardMask_Mean_RT_Long.xlsx", 
...
### Digit Span Info
Every item on Digit Span consists of two trials, each of which is scored 1 or 0 points. 
Pacing of the item stimuli is critical on Digit Span, as variations in how quickly or slowly 
they are provided can greatly impact item difficulty. The numbers are read at the rate of one 
per second, and there are a number of techniques that examiners use to establish an accurate, 
consistent pace.(https://www.sciencedirect.com/science/article/pii/B9780124046979000029)
...
Test if medication has an effect on N170 values

```{r}
data <- read_excel("/Users/julia/Desktop/Git/EEGfMRIStudy/data/erp_N170_MDD_On_vs_Off_Medication.xlsx")
head(data)
```
```{r}
# Factorise variables
...
```{r}
Model_medication_effect <- lmer(N170 ~ consciousness*primer_emotion
                          + gender 
                          + age
                          + Medication_1yes_2no
                          + (1|name),
...
```{r}
sjPlot::tab_model(Model_medication_effect)
```

```{r}
sjPlot::plot_model(Model_medication_effect, type = "diag")
```


```{r}
anova<-anova(Model_medication_effect, type = 3, ddf= "Kenward-Roger")
print(anova)
```
