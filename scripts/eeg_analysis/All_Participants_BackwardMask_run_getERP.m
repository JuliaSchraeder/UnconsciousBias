...
for i = 1:length(all_subjects)
    ...
    erp_happy(i,:) = Erp.happy;
    erp_neutral(i,:) = Erp.neutral;
    erp_sad(i,:) = Erp.sad;
...
y = mean(erp_strong,1);
x = erp_times;
std_dev = std(erp_strong);
curve1 = y + std_dev;
curve2 = y - std_dev;
x2 = [x, fliplr(x)];
inBetween = [curve1, fliplr(curve2)];
fill(x2, inBetween, 'blue','LineStyle',"none",'FaceAlpha',.3); hold on;
plot(x, mean(erp_strong,1),'Color','blue','LineWidth',1.5,'LineStyle','--'); 

%sd for weak
y = mean(erp_weak,1);
x = erp_times;
std_dev = std(erp_weak);
curve1 = y + std_dev;
curve2 = y - std_dev;
x2 = [x, fliplr(x)];
inBetween = [curve1, fliplr(curve2)];
fill(x2, inBetween, 'magenta','LineStyle',"none",'FaceAlpha',.3); 
plot(x,mean(erp_weak,1),'Color','magenta','LineWidth',1.5); hold off;
